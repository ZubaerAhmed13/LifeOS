...
const marker='data-decision-what-now-evidence="4.6.0"';
if(source.includes(marker)){
  console.log('LifeOS 4.6 Decision-backed What Now evidence migration already present.');
  process.exit(0);
}
...
const replacement=`  api.app.decisionEngine=engine;
  api.app.whatNow=async function(){
    try{
      ...
      if(!recommended){
        this.modal.open('What should I do now?',\`<div data-decision-what-now data-decision-what-now-evidence="4.6.0" class="note"><b>No feasible next action is available.</b><p>Review blocked work, protected recovery, fixed commitments and hard capacity constraints in Decision Center.</p></div>\`,'<button class="btn" data-action="close-dialog">Close</button>');
        return decision;
      }
...
}else if(source.includes('data-decision-what-now')){
  const replaceOnce=(oldValue,newValue,label)=>{
    const first=source.indexOf(oldValue);
    if(first<0)throw new Error(`LifeOS 4.6 What Now migration guard failed: ${label} signature missing.`);
    if(source.indexOf(oldValue,first+oldValue.length)>=0)throw new Error(`LifeOS 4.6 What Now migration guard failed: ${label} signature not unique.`);
    source=source.replace(oldValue,newValue);
  };
  replaceOnce(
    "      const startAction=candidate.kind==='task-session'&&candidate.taskId?",
    "      const intelligenceEvidence=array(recommended.tradeoffs?.reasons).find(item=>item.reasonCode==='PERSONAL-INTELLIGENCE');\n      const evidenceText=intelligenceEvidence?`Historical duration evidence — ${intelligenceEvidence.comparison||'Accepted Personal Intelligence was used as a soft ranking signal.'}`:'Limited historical evidence is available for this decision.';\n      const startAction=candidate.kind==='task-session'&&candidate.taskId?",
    'evidence computation'
  );
  replaceOnce(
    '<div data-decision-what-now><div class="success-box">',
    '<div data-decision-what-now data-decision-what-now-evidence="4.6.0"><div class="success-box">',
    'recommendation marker'
  );
  replaceOnce(
    "${why||'<li>The recommendation passed hard feasibility and deterministic trade-off ranking.</li>'}</ul><h3>What this protects</h3>",
    "${why||'<li>The recommendation passed hard feasibility and deterministic trade-off ranking.</li>'}</ul><h3>Evidence</h3><p>${escapeHtml(evidenceText)}</p><p class=\"muted\">No calendar changes required</p><h3>What this protects</h3>",
    'evidence UI'
  );
  source=source.replaceAll('<div data-decision-what-now class="note">','<div data-decision-what-now data-decision-what-now-evidence="4.6.0" class="note">');
  source=source.replaceAll('<div data-decision-what-now class="note warning">','<div data-decision-what-now data-decision-what-now-evidence="4.6.0" class="note warning">');
}else{
  throw new Error('LifeOS 4.6 What Now hardening guard failed: neither base nor prior 4.6 What Now implementation was found.');
}

fs.writeFileSync(file,source);
for(const invariant of [marker,"source:'what-now'",'DECISION ENGINE RECOMMENDATION','Historical duration evidence','Limited historical evidence','No calendar changes required','What this protects','Opportunity cost','Best alternative'])if(!source.includes(invariant))throw new Error(`LifeOS 4.6 What Now invariant missing: ${invariant}`);
