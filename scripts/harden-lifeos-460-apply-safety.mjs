...
'DATASET-SNAPSHOT-TX-460',
'coherent repository dataset snapshot in one IndexedDB transaction'
);
fs.writeFileSync(appFile,app);
for(const invariant of ['dataCacheGeneration=0','this.dataCacheGeneration+=1','DATASET-SNAPSHOT-TX-460'])if(!app.includes(invariant))throw new Error(`LifeOS 4.6 repository snapshot invariant missing: ${invariant}`);

console.log('LifeOS 4.6 apply safety and transactional coherent repository snapshot hardening applied and verified.');
