...
const replacement=`  api.app.decisionEngine=engine;
  api.app.whatNow=async function(){
    try{
      ...
      if(!recommended){
        this.modal.open('What should I do now?',\`<div data-decision-what-now class="note"><b>No feasible next action is available.</b><p>Review blocked work, protected recovery, fixed commitments and hard capacity constraints in Decision Center.</p></div>\`,'<button class="btn" data-action="close-dialog">Close</button><button class="btn primary" data-action="decision-center-open">Decision Center</button>');
        return decision;
      }
      ...
      const startAction=candidate.kind==='task-session'&&candidate.taskId?\`<button class="btn primary" data-action="start-focus" data-id="\${escapeHtml(candidate.taskId)}">Start Focus</button>\`:'<button class="btn primary" data-action="decision-center-open">Open Decision Center</button>';
