...
replaceOnce(
  ...
  "  // A clean test database requires onboarding to be completed before the test begins.\n  // Firefox can surface the known onboarding dialog after the first visibility probe while\n  // service-worker control settles, so drive that specific transition to its persisted state\n  // instead of racing the modal. Arbitrary dialogs are never auto-dismissed.\n  const onboarding = page.locator('#appDialog').filter({ hasText: 'Welcome to LifeOS 4.4' });\n  const skip = onboarding.getByRole('button', { name: 'Skip setup', exact: true });\n  await expect.poll(async () => {\n    if (await skip.isVisible().catch(() => false)) await skip.click();\n    return page.evaluate(async () => Boolean((await globalThis.LifeOS.app.repo.settings()).onboardingComplete));\n  }, { timeout: 15_000 }).toBe(true);\n  await expect(onboarding).not.toBeVisible();\n  await expect(page.locator('#appDialog')).not.toBeVisible();",
  'deterministic Firefox onboarding persistence in resetApp'
);
